...
#include <vector>
namespace pclp {
// SHA-256, FIPS 180-4. Used for reproducibility fingerprints, not authentication.
inline std::string sha256(std::string_view input) {
  ...
  std::uint64_t bits=static_cast<std::uint64_t>(bytes.size())*8;
  ...
  for(std::size_t offset=0;offset<bytes.size();offset+=64){
    ...
    for(std::size_t i=0;i<64;++i){
      auto t1=v+(std::rotr(e,6)^std::rotr(e,11)^std::rotr(e,25))+((e&f)^(~e&g))+k[i]+w[i];
  ...
  for(auto x:h)stream<<std::setw(8)<<x;
  return stream.str();
}
}
